...
async function main() {
  ...
  try {
    ...
    // 1. 获取当前日期
    const dateResult = await manager.callTool('12306-mcp', 'get-current-date', {});
    console.log('当前日期:', dateResult.content[0].text);
    
    // 2. 获取车站代码
    const guangzhouCode = await manager.callTool('12306-mcp', 'get-station-code-of-citys', {
      citys: '广州'
    });
    console.log('广州车站代码:', guangzhouCode.content[0].text);
    
    const yichangCode = await manager.callTool('12306-mcp', 'get-station-code-of-citys', {
      citys: '宜昌'
    });
    console.log('宜昌车站代码:', yichangCode.content[0].text);
    
    // 3. 查询火车票（简化版）
    console.log('\n🚄 查询火车票...');
    try {
      const tickets = await manager.callTool('12306-mcp', 'get-tickets', {
        date: '2026-02-17',
        fromStation: 'GZQ',
        toStation: 'YCN',
        trainFilterFlags: 'G',
      ...
      if (tickets.content && tickets.content[0]) {
        console.log('查询结果:');
        console.log(tickets.content[0].text);
      }
    } catch (error) {
      console.log('查询失败（需要正确的车站代码）:', error.message);
    }
    
    // 示例：调用文件系统工具
    console.log('\n📁 示例：调用文件系统工具');
    try {
      const files = await manager.callTool('filesystem', 'read', {
        uri: 'file://' + path.join(__dirname, 'workspace')
      });
      console.log('工作空间文件:', files.content[0]?.text?.substring(0, 200) || '无内容');
    } catch (error) {
      console.log('文件系统工具调用失败:', error.message);
    }
