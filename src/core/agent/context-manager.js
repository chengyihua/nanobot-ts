...
class ContextManager {
    constructor(config) {
        this.log = (0, logger_js_1.createLogger)('context-manager');
        this.config = config;
     ...
     */
    sanitizeHistory(history, isFinalForLLM = false) {
        const result = [];
        for (let i = 0; i < history.length; i++) {
            ...
            if (msg.role === 'assistant') {
                ...
                if (Array.isArray(msg.content)) {
                    toolCalls = msg.content.filter((c) => c.type === 'tool-call');
                }
                ...
                if (hasToolCalls) {
                    ...
                    if (hasFollowingTool) {
                        ...
                        if (isLastMessage) {
                            ...
                            result.push(msg);
                            const placeholderResults = toolCalls.map((tc) => ({
                                type: 'tool-result',
                ...
                const lastHasTC = lastMsg && lastMsg.role === 'assistant' && ((Array.isArray(lastMsg.content) && lastMsg.content.some((c) => c.type === 'tool-call')) ||
