...
generate_images() {
    ...
    # 生成图片
    if [ -f "$IMAGE_GENERATOR" ]; then
        "$IMAGE_GENERATOR" batch "$topics_file" "$IMAGE_DIR"
        
        # 检查生成的图片
        image_count=$(find "$IMAGE_DIR" -name "*.jpg" -o -name "*.png" 2>/dev/null | wc -l)
        
        if [ $image_count -eq ${#topics[@]} ]; then
            success "✅ 所有配图生成成功 ($image_count 张)"
            
            # 显示图片列表
            log "🖼️  生成的图片:"
            find "$IMAGE_DIR" -name "*.jpg" -o -name "*.png" 2>/dev/null | sort | while read img; do
                echo "  - $(basename "$img")"
            done
        else
            warning "⚠️  只生成 $image_count/${#topics[@]} 张图片"
        fi
    else
        error "❌ 图像生成器不存在"
        return 1
...
get_image_for_topic() {
    ...
    # 查找对应序号的图片
    image=$(find "$image_dir" -name "cover_${topic_index}.jpg" -o -name "cover_${topic_index}.png" 2>/dev/null | head -1)
    
    if [ -n "$image" ] && [ -f "$image" ]; then
        echo "$image"
    else
        # 尝试查找其他格式
        image=$(find "$image_dir" -name "*${topic_index}*.jpg" -o -name "*${topic_index}*.png" 2>/dev/null | head -1)
        if [ -n "$image" ] && [ -f "$image" ]; then
            echo "$image"
        else
            echo ""
...
$(if [ -n "$cover_image" ]; then echo "![封面图片]($(basename "$cover_image"))"; fi)
...
publish_articles() {
    ...
    for i in "${!articles_info[@]}"; do
        ...
        # 发布文章
        if [ -f "$AUTO_PUBLISH" ]; then
            if "$AUTO_PUBLISH" publish "$article_file" "$title" "$cover_image" "$author" "$summary"; then
                success "    ✅ 发布成功"
...
- **选题数量**: 3篇
- **配图生成**: $(find "$IMAGE_DIR" -name "*.jpg" -o -name "*.png" 2>/dev/null | wc -l)张
- **文章生成**: $(find "$ARTICLE_DIR" -name "*.md" 2>/dev/null | wc -l)篇
- **发布成功**: $(grep -c "发布成功" "$LOG_FILE" 2>/dev/null || echo "0")篇
