...
# 每日AI深度文章自动发布系统（带智能配图）
# 每天自动分析热点，创作2-3篇不同选题的深度文章，并生成匹配的封面图片

WORKSPACE="/Users/chengyihua/Downloads/nanobot-main/nanobot-ts/workspace"
AUTO_PUBLISH="$WORKSPACE/auto_publish_system.sh"
IMAGE_GENERATOR="$WORKSPACE/smart_image_generator.py"
DATE=$(date '+%Y-%m-%d')
LOG_FILE="$WORKSPACE/daily_publish_with_images_${DATE}.log"
IMAGE_DIR="$WORKSPACE/daily_images_${DATE}"
...
init() {
    ...
    # 创建图片目录
    mkdir -p "$IMAGE_DIR"
    log "🖼️  图片目录: $IMAGE_DIR"
    
    echo "$ARTICLE_DIR:$IMAGE_DIR"
}
...
generate_cover_image() {
    ...
    log "🎨 为文章生成匹配封面图片: $topic"
    
    if [ -f "$IMAGE_GENERATOR" ]; then
        # 使用Python脚本生成图片
        python3 "$IMAGE_GENERATOR" generate "$topic" "$image_dir" "$index"
        
        # 查找最新生成的图片
        latest_image=$(find "$image_dir" -name "cover_${index}_*.jpg" -o -name "cover_${index}_*.png" 2>/dev/null | sort -r | head -1)
        
        if [ -n "$latest_image" ] && [ -f "$latest_image" ]; then
            success "✅ 封面图片生成成功: $latest_image"
            echo "$latest_image"
        else
            warning "⚠️  未找到生成的图片，使用默认封面"
            echo ""
        fi
    else
        warning "⚠️  图片生成器不存在，使用默认封面"
        echo ""
    fi
}
...
generate_article() {
    ...
    if [ -n "$cover_image" ] && [ -f "$cover_image" ]; then
        cover_note="*配图: 根据'$topic'主题生成的AI封面图片*"
    fi
...
EOF
    ...
    echo "$article_file:$author:$cover_image"
}
...
main() {
    # 初始化
    IFS=':' read -r article_dir image_dir <<< "$(init)"
    ...
    for i in "${!topics[@]}"; do
        ...
        cover_image=$(generate_cover_image "$topic" "$index" "$image_dir")
    ...
    for i in "${!topics[@]}"; do
        ...
        # 生成文章
        result=$(generate_article "$topic" "$index" "$article_dir" "$cover_image")
        IFS=':' read -r article_file author actual_cover <<< "$result"
        
        if [ -f "$article_file" ]; then
            # 发布文章
            if publish_article "$article_file" "$author" "$actual_cover" "$index"; then
    ...
    # 显示生成的图片
    if [ -d "$image_dir" ]; then
        image_count=$(find "$image_dir" -name "*.jpg" -o -name "*.png" 2>/dev/null | wc -l)
        log "   🖼️  生成图片: $image_count 张"
        
        if [ $image_count -gt 0 ]; then
            log "   图片列表:"
            find "$image_dir" -name "*.jpg" -o -name "*.png" 2>/dev/null | while read img; do
                log "     - $(basename "$img")"
            done
        fi
    fi
