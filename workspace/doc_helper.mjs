...
class DocIndexHelper {
  constructor(indexPath = path.join(__dirname, 'DOC_INDEX.md')) {
  ...
  readDocument(filePath, maxLines = 100) {
    try {
      ...
      if (!actualPath.startsWith('/') && !actualPath.startsWith('skills/')) {
        actualPath = path.join(workspacePath, actualPath);
      } else if (actualPath.startsWith('/workspace/')) {
        actualPath = path.join(workspacePath, actualPath.replace('/workspace/', ''));
      }
